...
if [[ $# != 1 ]]; then
  echo "USAGE: bumpTemplateVersion.sh <react native version>"
  exit 1
fi
...
if [[ $1 =~ [0-9]+\.[0-9]+\.[0-9]+ ]]; then 
  ...
  # Bump PATCH
  PUBLISHED=$(jq -r '.version' <<< $CURRENT_VERSION | awk -F'.' '{ print $1"."$2"."($3+1) }')
  echo $PUBLISHED;
  exit 0
fi

echo "Unknown type of release: $RN_VERSION"
exit 1
